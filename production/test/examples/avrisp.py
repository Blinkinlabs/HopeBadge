...
class AvrISP:
    ...
    def checkSignature(self):
        ...
        return ((responses[3] == self.signature[0]) and (
            responses[7] == self.signature[1]) and (responses[11] == self.signature[2]))
    ...
    def writePage(self, address, data):
        ...
        for i in range(0, self.pageSize):
            self.spi.xfer2([0x40, (((address + i) >> 8) & 0xFF),
                            ((address + i) & 0xFF), data[i * 2]])
            self.spi.xfer2([0x48, (((address + i) >> 8) & 0xFF),
                            ((address + i) & 0xFF), data[i * 2 + 1]])

        self.spi.xfer2(
            [0x4C, ((address >> 8) & 0xFF), (address & 0xFF), 0x0A0])
        time.sleep(ATTINY_FLASH_DELAY)
        return True

    def readWord(self, address):
        word = []
        word.append(self.spi.xfer2(
            [0x20, ((address >> 8) & 0xFF), (address & 0xFF), 0x00])[3])
        word.append(self.spi.xfer2(
            [0x28, ((address >> 8) & 0xFF), (address & 0xFF), 0x00])[3])
        return word
    ...
    def writeAndVerifyFuse(self, fuse, value):
        ...
        if (value != newValue):
            print(
                "Fuse value mismatch, expected 0x%x, got 0x%x" %
                (value, newValue))
            return False
